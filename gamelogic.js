...
export class GameLogic {
	...
	createSineWavePlatform(x, y, z) {
		...
		console.log(
			"First world triangle Y bounds:",
			worldTriangles[0].bounds.minY,
			"to",
			worldTriangles[0].bounds.maxY,
		);
	}
	...
	update() {
		...
		if (Math.random() < 0.01) {
			const nearby = this.collisionManager.getNearbyTriangles(
				player.position,
				player.radius,
			);
			console.log(
		...
		if (player.position.x > this.currentFloorX + 5) {
			...
			if (this.floorCounts % 10 === 0 && this.floorCounts !== 0) {
				...
				this.createZigZagPlatform(this.currentFloorX, 0, 0);
			}
