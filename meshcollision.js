...
export class MeshCollider {
    ...
    extractTriangles() {
        ...
        if (indices) {
            for (let i = 0; i < indices.length; i += 3) {
                ...
                triangles.push({
                    v0: new THREE.Vector3(positions[i0], positions[
                        i0 + 1], positions[i0 + 2]),
                    v1: new THREE.Vector3(positions[i1], positions[
                        i1 + 1], positions[i1 + 2]),
                    v2: new THREE.Vector3(positions[i2], positions[
                        i2 + 1], positions[i2 + 2])
                });
            }
        } else {
            for (let i = 0; i < positions.length; i += 9) {
                triangles.push({
                    v0: new THREE.Vector3(positions[i], positions[
                        i + 1], positions[i + 2]),
                    v1: new THREE.Vector3(positions[i + 3],
                        positions[i + 4], positions[i + 5]),
                    v2: new THREE.Vector3(positions[i + 6],
                        positions[i + 7], positions[i + 8])
                });
            }
        }
    ...
    collideSphere(sphere, friction = 0.5, restitution = 0.7, dt = 0.016) {
        ...
        const nearby = this.getNearbyTriangles(sphere.position, sphere
            .radius, 5);
