...
export class GameLogic {
    ...
    createSineWavePlatform(x, y, z) {
        ...
        const worldTriangles = result.triangles.map(tri => {
            const v0 = tri.v0.clone().applyMatrix4(result.mesh
                .matrixWorld);
            const v1 = tri.v1.clone().applyMatrix4(result.mesh
                .matrixWorld);
            const v2 = tri.v2.clone().applyMatrix4(result.mesh
                .matrixWorld);

            const edge1 = new THREE.Vector3().subVectors(v1, v0);
            const edge2 = new THREE.Vector3().subVectors(v2, v0);
            const normal = new THREE.Vector3().crossVectors(edge1,
                edge2).normalize();
        ...
        console.log('First world triangle Y bounds:', worldTriangles[0]
            .bounds.minY, 'to', worldTriangles[0].bounds.maxY);
    }
    ...
    update() {
        ...
        if (Math.random() < 0.01) {
            const nearby = this.collisionManager.getNearbyTriangles(player
                .position, player.radius);
            console.log(
        ...
        if (player.position.x > this.currentFloorX + 5) {
            ...
            if (this.floorCounts % 5 === 0 && this.floorCounts !== 0) {
                this.floorManager.createFloor(
                    this.currentFloorX, 0, 0, 10, 1, 10, 0, 0, 0
                );
            } else {
                this.floorManager.createFloor(
                    this.currentFloorX, 0, 0, 10, 1, 10, 0, 0, 0
